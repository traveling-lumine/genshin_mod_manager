...
void main() {
  group('obtainValue', () {
    test('default data', () {
      const config = AppConfigFacadeImpl(config: AppConfig({}));
      final entry = stringEntry(key: 'key', defaultValue: 'default');
      expect(config.obtainValue(entry), 'default');
    });
    test('retrieve data', () {
      const config = AppConfigFacadeImpl(config: AppConfig({'key': 'value'}));
      final entry = stringEntry(key: 'key', defaultValue: 'default');
      expect(config.obtainValue(entry), 'value');
    });
    test('invalid data', () {
      const config = AppConfigFacadeImpl(config: AppConfig({'key': 1}));
      final entry = stringEntry(key: 'key', defaultValue: 'default');
      expect(config.obtainValue(entry), 'default');
    });
  });
